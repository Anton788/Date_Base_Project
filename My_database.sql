...
CREATE VIEW PARTY_V_TOWN
    AS (SELECT party_type_code, first_nm, second_nm, (CASE
        WHEN
            ...
            ELSE
              birth_dt
        END), town_nm, education_coeff
    FROM PARTY INNER JOIN TOWN
...
CREATE VIEW POLICE_D
    AS (SELECT address_txt, CASE
        WHEN
            TRUE THEN SUBSTRING(phone_no, 1, 2) || '*******' || SUBSTRING(phone_no, 10, 2)
        END, close_case_coeff
...
CREATE VIEW LIBRARY_V AS (
    SELECT address_txt, CASE
        WHEN
            TRUE THEN SUBSTRING(phone_no, 1, 2) || '*******' || SUBSTRING(phone_no, 10, 2) END, main_book_nm
    FROM LIBRARY);
...
CREATE VIEW SCHOOL_V_TOWN AS (
    SELECT school_no, address_txt, grant_amt, yard_flg, CASE
        WHEN
            sponsor_nm NOTNULL THEN SUBSTRING(sponsor_nm, 0, 3) || '****'
        ELSE
            sponsor_nm
        END, town_nm, education_coeff
    FROM SCHOOL
