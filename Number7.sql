...
CREATE VIEW TOWN_V AS
    SELECT town_nm, mayor_nm, (CASE
        WHEN education_coeff < 50 THEN NULL
        ELSE education_coeff END )
    FROM TOWN;
...
CREATE VIEW PARTY_V
    AS SELECT party_type_code, first_nm, second_nm, (CASE
        WHEN  party_type_code = 'Учитель' THEN SUBSTRING(birth_dt, 0, 3) || '*******' || SUBSTRING(birth_dt, 8, 3)
            ELSE
              birth_dt
        END)
    FROM PARTY;
...
CREATE VIEW POLICE_D
    AS SELECT address_txt, CASE
        WHEN TRUE THEN SUBSTRING(phone_no, 1, 2) || '*******' || SUBSTRING(phone_no, 10, 2) END, close_case_coeff
...
CREATE VIEW LIBRARY_V AS
    SELECT address_txt, CASE
        WHEN TRUE THEN SUBSTRING(phone_no, 1, 2) || '*******' || SUBSTRING(phone_no, 10, 2) END, main_book_nm
    FROM LIBRARY;
